...
{
    int **initmat,**predecessor,v,k;
    initmat = new int*[nVertices];
    predecessor=new int*[nVertices];
    for(int i=0; i<nVertices; i++)
    {
        initmat[i] = new int[nVertices];
        predecessor[i]=new int[nVertices];
        for(int j=0; j<nVertices; j++)
        {
            initmat[i][j] = INFINITY;
            predecessor[i][j]=-1;
        }
    }
    for(int i=0; i<nVertices; i++)
    {
        for(int j=0; j<adjList[i].getLength(); j++)
        {
            ...
            initmat[i][v]=matrix[i][v];
            predecessor[i][v]=i;
            //   cout<<matweight[i][v]<<' '<<i+1<<' '<<v+1<<endl;
        }
    }
    ...
    for(int i=0; i<nVertices; i++)
    {
        for(int j=0; j<nVertices; j++)
        {
            if(i==j) initmat[i][j]=0;
            //  if(j==v) initmat[i][j]=matrix[i][v];
            cout<<initmat[i][j]<<" ";
        }
        cout<<endl;
    }
    for(int k=0; k<nVertices; k++)
    {
        for(int i=0; i<nVertices; i++)
        {
            for(int j=0; j<nVertices; j++)
            {
                if(initmat[i][j]>initmat[i][k]+initmat[k][j])
                {
                    initmat[i][j]=initmat[i][k]+initmat[k][j];
